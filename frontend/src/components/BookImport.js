...
const BookImport = () => {
  ...
  const handlePreview = async () => {
    ...
    try {
      const response = await adminBooksAPI.previewBookImport(formData);
  ...
  const handleImport = async () => {
    ...
    try {
      const response = await adminBooksAPI.executeBookImport({
  ...
  return (
    <div className="book-import" style={{ padding: '2rem' }}>
      ...
      <p style={{ color: '#666', marginBottom: '2rem' }}>
        Import your book inventory using ISBNs. Book details will be automatically fetched from ISBNDB, Google Books, or Open Library, with CSV data as fallback.
      </p>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <div style={{ display: 'flex', gap: '1rem', marginBottom: '1rem' }}>
          <button
            onClick={() => setMode('csv')}
            className={mode === 'csv' ? 'btn btn-primary' : 'btn'}
          >
            Import from CSV
          </button>
          <button
      ...
      {mode === 'manual' && (
        <div style={{ marginBottom: '2rem' }}>
          <h3>Enter ISBNs</h3>
          <p style={{ color: '#666' }}>Enter one ISBN per line or separate with commas</p>
          <textarea
            value={isbnList}
